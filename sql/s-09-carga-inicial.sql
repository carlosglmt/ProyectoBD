    	...
    	v_posicion := instr(v_linea,'#',1,1);
			...
			if p_nombre_tabla = 'marca' then
				...
				insert into usuario(usuario_id,username,nombre,apellido_paterno,
						apellido_materno,email,clave_acceso,es_administrador,
						es_conductor,es_cliente,descuento,usuario_existente)
				values (usuario_seq.nextval,a_datos(1),a_datos(2),a_datos(3),
					a_datos(4),a_datos(5),a_datos(6),a_datos(7),a_datos(8),a_datos(9),
					to_number(a_datos(10)),to_number(a_datos(11)));
				...
				--Si es conductor y es cliente, entonces inserta en la tabla conductor y cliente
					...
					insert into cliente(usuario_id,fecha_registro,num_celular)
					values(usuario_seq.currval,nvl(a_datos(12),sysdate),a_datos(13));
					insert into conductor(usuario_id,num_licencia,
							num_cedula,foto,descripcion)
					values(usuario_seq.currval,a_datos(14),a_datos(15),
							hextoraw(a_datos(16)),a_datos(17));
				...
				--Si sólo es conductor, entonces inserta en la tabla conductor. 
					...
					insert into conductor(usuario_id,num_licencia,num_cedula,
							foto,descripcion)
					values(usuario_seq.currval,a_datos(12),a_datos(13),
							hextoraw(a_datos(14)),a_datos(15));
				...
				values(factura_seq.nextval,nvl(a_datos(1),sysdate),a_datos(2),
						hextoraw(a_datos(3)));
				...
				insert into viaje(viaje_id,latitud_origen,latitud_destino,
						longitud_origen,longitud_destino,importe,hora_inicio,hora_fin,
						propina,comentario,calificacion,fecha_status,usuario_id,auto_id,
						factura_id,status_viaje_id)
				values (viaje_seq.nextval,a_datos(1),a_datos(2),a_datos(3),
						a_datos(4),a_datos(5),nvl(to_date(a_datos(6),'dd/mm/yyyy hh24:mi:ss'),sysdate),
						nvl(to_date(a_datos(7),'dd/mm/yyyy hh24:mi:ss'),sysdate),
						a_datos(8),a_datos(9),a_datos(10),nvl(to_date(a_datos(11),'dd/mm/yyyy hh24:mi:ss'),
							sysdate),a_datos(12),a_datos(13),a_datos(14),a_datos(15));
