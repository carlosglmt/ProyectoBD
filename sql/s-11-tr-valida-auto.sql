...
declare
  v_temp_num_autos number(1,0);
  v_perm_num_autos number(1,0);
...
begin
  ...
  select count(*) into v_temp_num_autos
  from temp_auto
  where usuario_id = :new.usuario_id;
  select count(*) into v_perm_num_autos
  from auto
  where usuario_id = :new.usuario_id;  
  if v_temp_num_autos + v_perm_num_autos >= 2 then
